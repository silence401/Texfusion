...
def _get_nvdiff_glctx(device, return_derivatives=False):
    if device not in _device2glctx:
        _device2glctx[device] = nvdiff.RasterizeGLContext(
            output_db=return_derivatives, device=device)
...
def _nvdiff_rasterize(height,
    ...
    glctx = _get_nvdiff_glctx(device, return_derivatives=return_derivatives)
    ...
    return interpolated_features, face_idx, rast, pos, tri
...
def rasterize(height,
    ...
    if backend == 'nvdiffrast':
        image_features, face_idx, rast, pos, tri = _nvdiff_rasterize(
            height, width, face_vertices_z, face_vertices_image,
            _face_features, valid_faces, return_derivatives=return_derivatives)
        
    elif backend == 'nvdiffrast_fwd':
        image_features, face_idx, rast, pos, tri = NvdiffRasterizeFwdCudaBwd.apply(
            height, width, face_vertices_z, face_vertices_image,
    ...
    if backend == 'nvdiffrast':
        return image_features, face_idx, rast, pos, tri
