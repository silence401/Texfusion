...
def scale_verts(verts, max_l, center):
    scale_verts = verts / 0.9
    scale_verts = scale_verts * max_l
    scale_verts = scale_verts + center
    return scale_verts
...
def depth_to_normal(depth):
    ...
    normal_y = normal_tmp[:, :, 1, ...]
    #import pdb; pdb.set_trace()
    z = torch.ones_like(normal_x) * 0.006#((normal_x.max() - normal_x.min())/2 / np.pi)
    #normal_x[uv_depth < 0.1] = 0
    #normal_y[uv_depth < 0.1] = 0
    normal = torch.cat([normal_x, normal_y, z], dim=1)
