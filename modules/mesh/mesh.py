...
class Mesh:
    """Base mesh class"""
    def __init__(self, v_pos=None, t_pos_idx=None, \
                v_nrm=None, t_nrm_idx=None, v_tex=None,\
                t_tex_idx=None, v_tng=None, t_tng_idx=None, material=None, base=None):
        """__init__"""
        self.v_pos = v_pos
        self.v_nrm = v_nrm
        self.v_tex = v_tex
    ...
    def normalize_mesh(self, inplace=False, target_scale=1, dy=0):
        """
        Normalize the mesh according to its vertex position 
        """
...
def load_mesh(filename, mtl_override=None, device='cuda'):
    """Mesh loeading helper"""
    name, ext = os.path.splitext(filename)
    if ext == ".obj":
        return obj.load_obj(filename, clear_ks=True, mtl_override=mtl_override, device=device)
...
def rescale(mesh):
    ...
    mesh.v_pos = (mesh.v_pos - center) / max_l * 0.9
    return mesh
######################################################################################
...
def auto_normals(imesh):
    ...
    v_nrm = torch.where(util.dot(v_nrm, v_nrm) > 1e-20, \
            v_nrm, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device='cuda'))
