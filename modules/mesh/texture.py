...
class Texture2dMip(torch.autograd.Function):
    """Mip Texture"""
    @staticmethod
    ...
    def backward(ctx, dout):
        """backward"""
        gy, gx = torch.meshgrid(torch.linspace(0.0 + 0.25 / dout.shape[1], 
                                        1.0 - 0.25 / dout.shape[1], dout.shape[1] * 2, device="cuda"), 
                                torch.linspace(0.0 + 0.25 / dout.shape[2], 
                                        1.0 - 0.25 / dout.shape[2], dout.shape[2] * 2, device="cuda"))
        ...
        if isinstance(self.data, list):
            ...
            if self.data.shape[1] > 1 and self.data.shape[2] > 1:
                ...
                while mips[-1].shape[1] > 1 and mips[-1].shape[2] > 1:
                    mips += [Texture2dMip.apply(mips[-1])]
