...
def load_mtl(fn, clear_ks=True):
    ...
    for mat in materials:
        ...
        if 'bump' in mat:
            mat['normal'] = texture.load_texture2D(os.path.join(mtl_path, mat['bump']), \
                    lambda_fn=lambda x: x * 2 - 1, channels=3)
    ...
    with open(fn, "w") as f:
        ...
        if material is not None:
            ...
            if 'normal' in material.keys():
                ...
                texture.save_texture2D(os.path.join(folder, 'texture_n.png'), \
                        material['normal'], lambda_fn=lambda x:(util.safe_normalize(x) + 1) * 0.5)
        else:
            f.write('Kd 1 1 1\n')
            f.write('Ks 0 0 0\n')
            f.write('Ka 0 0 0\n')
...
def merge_materials(materials, texcoords, tfaces, mfaces):
    ...
    for mat in materials:
        ...
        assert ('normal' in mat) is ('normal' in materials[0]), \
    ...
    for tex in textures:
        if tex in materials[0]:
            tex_data = torch.cat(tuple(util.scale_img_nhwc(mat[tex].data, \
                    tuple(max_res)) for mat in materials), dim=2) # Lay out all textures horizontally, NHWC so dim2 is x
    ...
    for fi in range(len(tfaces)):
        ...
        for vi in range(3):
            ...
            if not (matIdx in new_tverts[ti]): # create new vertex
                new_tverts_data.append([(matIdx + texcoords[ti][0]) / s_coeff[1], \
                        texcoords[ti][1] / s_coeff[0]]) # Offset texture coodrinate (x direction) by material id & scale to local space. Note, texcoords are (u,v) but texture is stored (w,h) so the indexes swap here
